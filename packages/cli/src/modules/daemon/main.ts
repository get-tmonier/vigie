...
import { createEventBus } from '../terminal/event-bus.js';
import { createTerminalService } from '../terminal/terminal.service.js';
import { createTerminalSubscribers } from '../terminal/terminal-subscribers.js';
...
import { createRoutesLayer } from './server/server.js';
...
export const runDaemon = Effect.gen(function* () {
  ...
  const uiDistCandidates = [
    join(dirname(process.execPath), 'ui-dist'),
    join(dirname(process.execPath), '..', 'ui-dist'),
