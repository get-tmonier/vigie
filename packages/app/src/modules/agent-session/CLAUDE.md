...
- Session live feed (real-time output streaming to viewers) — via `SessionFeed` port
- Agent adapters (`AgentAdapter` interface + `AgentCatalog`) — spawn config and resumability per agent
...
| `session-feed.port.ts` | `SessionFeedShape` | Real-time output streaming to viewers |
| `session-event-bus.port.ts` | `SessionEventBusShape` | Session lifecycle event pub/sub |
| `agent-adapter.port.ts` | `AgentAdapter` / `AgentCatalogShape` | Spawn config + resumability per agent |
...
Agent outputs bytes
  ...
  │         │
  │         ├─→ SessionFeed.publish()         — live WebSocket fans to browser viewers
  │         │
