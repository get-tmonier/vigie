...
function eventKey(event: DaemonEvent, index: number): string {
  if ('id' in event) return `${event.type}-${String(event.id)}-${index}`;
  if ('sessionId' in event) return `${event.type}-${String(event.sessionId)}-${index}`;
